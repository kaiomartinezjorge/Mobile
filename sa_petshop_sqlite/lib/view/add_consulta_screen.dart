...
import 'package:sa_petshop_sqlite/model/pet.dart';
import 'package:sa_petshop_sqlite/model/consulta.dart';
import 'package:sa_petshop_sqlite/view/pet_detail_screen.dart';
...
class _AddConsultaScreenState extends State<AddConsultaScreen> {
  ...
  void _dataSelecionada(BuildContext context) async {
    ...
    final DateTime? picked = await showDatePicker(
      context: context,
      firstDate:
          DateTime.now(), //não permite agendamento de data anterior ao dia atual
          ...
          child: ListView(
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: Text("Data: ${dataFormatada.format(_selectedDate)}"),
                  ),
                  TextButton(
              ...
              Row(
                children: [
                  Expanded(
                    child: Text(
                      "Data: ${horaFormatada.format(DateTime(0, 0, 0, _selectedTime.hour, _selectedTime.minute))}",
                    ),
                  ),
                  TextButton(
                    onPressed: () => _horaSelecionada(context),
